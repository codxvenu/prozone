...
function bins() {
  ...
  const [isloader, setIsloader] = useState(false);
    ...
    const handleSubmit = async (e) => {
      isloader(true)
        e.preventDefault();
        try {
          const response = await fetch('${process.env.NEXT_PUBLIC_BASE_API_URL}api/bins', {
            method: 'POST',
            headers: {
  ...
  const handleRowSelect = (id) => {
    ...
    const handleorder = async () => {
      ...
      try {
        const response = await fetch('${process.env.NEXT_PUBLIC_BASE_API_URL}api/purchase_bins', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          },
